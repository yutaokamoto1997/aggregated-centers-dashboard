...
function processSingleDayBatch(dateStr) {
  try {
    ...
    console.error(`${dateStr} 失敗: ${e.message}`);
  }
}
...
function runAggregationForSpecificPeriod() {
  ...
  const dates = new Set(),
    regex = new RegExp(`^${FOLDERS.input.workVolume.prefix}(\\d{4}-\\d{2}-\\d{2})\\.csv$`);
  while (files.hasNext()) {
  ...
  if (targetDates.length === 0)
    return console.log(`指定期間 (${TARGET_START} ～ ${TARGET_END}) のデータが見つかりません。`);

  const props = PropertiesService.getScriptProperties();
  const processed = props.getProperty(PROCESSED_DATES_KEY)
    ? JSON.parse(props.getProperty(PROCESSED_DATES_KEY))
    : [];
  ...
  if (pending.length === 0) {
    ...
    ScriptApp.getProjectTriggers().forEach((t) => {
      if (t.getHandlerFunction() === TRIGGER_FUNC) ScriptApp.deleteTrigger(t);
    });
    return console.log(
      `★ 指定期間 (${TARGET_START} ～ ${TARGET_END}) の集計はすべて完了しています。`
    );
  }
...
function getCsvDataFromInputBlob(type, opts) {
  ...
  for (let n of opts) {
    const files = f.searchFiles(
      `title = '${FOLDERS.input[type].prefix}${n}${FOLDERS.input[type].suffix || '.csv'}'`
    );
...
function uploadCsv(folderId, name, rows, headers) {
  const f = DriveApp.getFolderById(folderId),
    files = f.searchFiles(`title = '${name}'`);
  const content =
    '\uFEFF' +
  ...
  else f.createFile(Utilities.newBlob(content, 'text/csv', name));
}
...
function createDailyCsv(dateStr) {
  ...
  for (let i = 0; i < 12; i++) {
    const files = DriveApp.getFolderById(FOLDERS.input.cost.id).searchFiles(
      `title = '${FOLDERS.input.cost.prefix}${Utilities.formatDate(new Date(dt.getFullYear(), dt.getMonth() - i, 1), 'JST', 'yyyyMM')}.csv'`
    );
    if (files.hasNext()) {
      const file = files.next();
      raw.cost = { data: parseBlobSafely(file), name: file.getName() };
      break;
    }
  }
  ...
  const getE = (c, t) => {
    let k = `${c}-${t}`;
    if (!aggMap[k])
      aggMap[k] = { ih: 0, ic: 0, iv: 0, tmh: 0, tmc: 0, tmv: 0, oh: 0, oc: 0, ov: 0 };
    return aggMap[k];
  };
  const staffMap = {
    YSS: 'ＹＳＳ作業',
  ...
  raw.mp.data.forEach((r) => {
    ...
    let e = getE(c, t),
      m = getNum(r['投入時間']);
    e.ih += m;
    e.ic += getInt(r['人員数']) || getInt(r['投入人数']);
    if (staffMap[s] && costMap[c] && costMap[c][staffMap[s]])
      e.iv += (m / 60) * costMap[c][staffMap[s]];
  });
  ...
  [
    ...
    raw[cfg.k].data.forEach((r) => {
      ...
      let e = getE(c, t);
      e[`${cfg.p}h`] +=
        (getNum(r['投入時間']) || getNum(r['作業時間']) || getNum(r['委託時間']) || 0) * 60;
      e[`${cfg.p}c`] +=
        getInt(r['投入人数']) ||
        getInt(r['人員数']) ||
        getInt(r['利用人数']) ||
  ...
  raw.wv.data.forEach(
    (r) =>
      (volMap[`${r['集約拠点コード'] || r['拠点コード']}-${r['時間帯']}`] = getInt(r['作業個数']))
  );
  ...
  [...new Set([...Object.keys(volMap), ...Object.keys(aggMap)])].forEach((k) => {
    let [c, t] = k.split('-'),
      n = nameMap[c] || `(名称不明:${c})`;
    let vol = volMap[k] || 0,
      e = aggMap[k] || { ih: 0, ic: 0, iv: 0, tmh: 0, tmc: 0, tmv: 0, oh: 0, oc: 0, ov: 0 };

    if (vol === 0 && e.ih === 0 && e.ic === 0 && e.oh === 0 && e.oc === 0) return;

    let th = e.ih + e.tmh + e.oh,
      tc = e.ic + e.tmc + e.oc,
      tv = e.iv + e.tmv + e.ov;
    rows.push([
      dateStr,
...
function updatePeriodTotals(dateStr) {
  const t = new Date(dateStr),
    monDt = new Date(new Date(t).setDate(t.getDate() - t.getDay() + (t.getDay() === 0 ? -6 : 1)));
  const sunDt = new Date(monDt.getTime());
  sunDt.setDate(sunDt.getDate() + 6);
  const fstDt = new Date(t.getFullYear(), t.getMonth(), 1),
    lstDt = new Date(t.getFullYear(), t.getMonth() + 1, 0);

  const getDts = (s, e) => {
    let a = [];
    for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) a.push(formatDate(d));
    return a;
  };

  const weekConcat = concatDailyFiles(
    getDts(monDt, sunDt),
    FOLDERS.output.weekly_full,
    `${CSV_PREFIX.weekly_full}${formatDate(monDt)}.csv`
  );
  const monthConcat = concatDailyFiles(
    getDts(fstDt, lstDt),
    FOLDERS.output.monthly_full,
    `${CSV_PREFIX.monthly_full}${formatMonth(fstDt)}.csv`
  );

  buildRunningFromConcat(
    weekConcat,
    dateStr,
    formatDate(monDt),
    formatDate(sunDt),
    7,
    FOLDERS.output.weekly_running,
    CSV_PREFIX.weekly_running,
    formatDate(monDt)
  );
  buildRunningFromConcat(
    monthConcat,
    ...
    formatDate(lstDt),
    lstDt.getDate(),
    FOLDERS.output.monthly_running,
    CSV_PREFIX.monthly_running,
    formatMonth(fstDt)
  );
}
...
function buildRunningFromConcat(
  concatRows,
  refDate,
  startD,
  endD,
